...
if (isset($_GET['updatetherapist_id']) && isset($_POST['submit'])) {
    ...
    $stmt = $conn->prepare("UPDATE therapists SET full_name=?,  email=?, telephone=?,specialization=? WHERE therapist_id=?");
    $stmt->bind_param("sssi", $full_name, $email, $phone, $c_id);

    if ($stmt->execute()) {
        header('Location: therapists.php?msg=Record updated successfully');
        exit();
    } else {
...
if (isset($_GET['updatetherapists_id'])) {
    $c_id = $_GET['updatetherapists_id'];
    $sql_select = "SELECT * FROM therapists WHERE therapist_id=$c_id";
    ...
    <div class="container">
        ...
        <form method="POST" action="" onsubmit="return confirmUpdate();">
            ...
            <div class="form-group">
                ...
                <input type="text" id="specialization" name="specialization" value="<?php echo $row['specialization']; ?>" required>
            </div>
          
            <div class="form-group">
                <input type="submit" name="submit" value="Update" class="btn btn-primary">
                <a href="therapists.php" class="btn btn-secondary">Cancel</a>
            </div>
        </form>
    </div>
    <script>
