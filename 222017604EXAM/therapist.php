...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO therapists (full_name, email, Specialization) VALUES (?,  ?, ?)");
    ...
    // Bind parameters
    $stmt->bind_param("sss", $full_name, $email,  $speciality);
...
$sql_select = "SELECT * FROM therapists";

// Check if search form is submitted
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['search'])) {
    $search = $_GET['search'];
    // Add search condition to SQL query
    $sql_select .= " WHERE therapist_id LIKE '%$search%' OR full_name LIKE '%$search%' OR email LIKE  '%$search%' OR Specialization LIKE '%$search%' ";
}
...
<label class="logo" >otp <br>
    ...
    <div class="container">
        ...
        ?>
        <a href="therapist.html" class="btn btn-success">Add New</a>
    </div>
    <br>
    ...
    <table id="dataTable" class="table table-hover text-center">
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<td>";
                echo "<a href='therapistsupdate.php?updatesession_id=" . $row['therapist_id'] . "'><i class='fas fa-edit'></i></a>";
                echo "<a href='therapiststdelete.php?therapist_id=" . $row['therapist_id'] . "'><i class='fas fa-trash'></i></a>";
                echo "</td>";
                echo "</tr>";
            }
        } else {
